...
def fix_datasets_import():
    ...
    try:
        ...
        if not hasattr(features, '_FEATURE_TYPES'):
            print("🔧 [model_trainer] _FEATURE_TYPES不存在，开始修复...")
            ...
            print(f"🔧 [model_trainer] 已创建_FEATURE_TYPES ({len(_FEATURE_TYPES)}个类型)")

        # 修复exceptions模块
        if not hasattr(datasets, 'exceptions'):
            print("🔧 [model_trainer] exceptions模块不存在，开始修复...")
            import types
        ...
        if not hasattr(load, 'HubDatasetModuleFactoryWithParquetExport'):
            ...
            print("🔧 [model_trainer] 已创建HubDatasetModuleFactoryWithParquetExport兼容类")

        # 修复_get_importable_file_path
        if not hasattr(load, '_get_importable_file_path'):
            print("🔧 [model_trainer] _get_importable_file_path不存在，开始修复...")
